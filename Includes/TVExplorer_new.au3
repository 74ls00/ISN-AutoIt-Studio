...
Func _GUICtrlTVExplorer_Create($sRoot, $iX, $iY, $iWidth = -1, $iHeight = -1, $iStyle = -1, $iExStyle = 0, $iFlags = -1, $sFunc = '', $sTemplate = '',$verwende_alten_Treeview=0,$Mit_Checkboxen=0)
...
;~		EndIf
	...
	if $verwende_alten_Treeview = 1 Then
	$ID = $hTreeview;
   else
	$ID = GUICtrlCreateTreeView($iX, $iY, $iWidth, $iHeight,$iStyle,$iExStyle)
   EndIf
	GUICtrlSetStyle($ID , Default, BitOR($WS_EX_COMPOSITED, $WS_EX_CLIENTEDGE))
	If Not $ID Then
		For $i = 2 To 8
			If $tvData[$Index][$i] Then
...
Func _GUICtrlTVExplorer_Destroy($hTV,$wirklich_zerstoeren=0)
	...
	For $i = 2 To 8
		GUICtrlDelete($tvData[$Index][$i])
	Next
	if $wirklich_zerstoeren = 1 then GUICtrlDelete($tvData[$Index][29]) ;Nur löschen wenn $wirklich_zerstoeren 1 ist
...
;~ Eigene Icons für zb. .isf Dateien
   ...
   if _WinAPI_PathAddBackslash ($sPath) = _WinAPI_PathAddBackslash ($Projectfolder) then ;Icon für ISN AutoIt Studio Projekte
	$tIcon = DllStructCreate("int Icon")
    $pIcon = DllStructGetPtr($tIcon)
...
;~				$hRoot = _GUICtrlTreeView_AddChild($tvData[$iIndex][0], 0, _WinAPI_PathStripPath($sPath), $Image, $Image)
			...
			EndIf
		 Until 1
		 if $AutoIt_Projekte_in_Projektbaum_anzeigen = "true" then _TV_AddVirtualDrive($iIndex, $Projectfolder,_Get_langstr(881)) ;Shortcut in den Projekte Ordner
	  Else
		...
		If IsArray($Drive) Then
			...
			Next
		 EndIf
		 _TV_AddVirtualDrive($iIndex, $Projectfolder,_Get_langstr(881)) ;Shortcut in den Projekte Ordner
	EndIf
...
;		$Path = StringRegExpReplace(_GUICtrlTreeView_GetTree($tvData[$iIndex][0], $hItem), '([|]+)|(\\[|])', '\\')
	...
	if StringTrimRight($Path,stringlen($Path)-StringInStr($Path,"\")+1) = _Get_langstr(881) then
	   $Path = StringReplace($Path,_Get_langstr(881),$Projectfolder)
	   $Is_Shortcut = 1
    EndIf
   Else
	if $Path = $Projectfolder then $Is_Shortcut = 1
	endif
...
Func _LVMain()
	...
	_GUICtrlListView_SetImageList($listview , $tvData[0][1],1)
    GUISetState()
